...
public class ObjectNode
{
     ...
     *
     * pre-condition for listCopy()
     *   the ObjectNode source.link should not be empty, otherwise it will throw
     *   an OutOfMemoryError
     *
     **/
     ...
     * @return
     *  the method has made a copy of the linked list starting at
     *  source. The return value is the head reference for the copy
     * @exception OutOfMemoryError
     *  Indicates that there is insufficient memory for the new list
     *
     * runtime is theta(n)
    ...
    public static ObjectNode listCopy_rec(ObjectNode source)
    {
        ...
        ObjectNode temp = new ObjectNode(source.data, null);
        temp.link = listCopy_rec(source.link);
        return temp;
     ...
     *   Indicates that there is insufficient memory for the new list.
     *
     * pre-condition for listCopyWithTail()
     *   the ObjectNode source.link should not be empty, otherwise it will
...
//            cursor = cursor.link;
     ...
     * @return
     *  the whole string which consists of all data in ObjectNode
     */
    @Override
    public String toString()
    {
        ...
        while (temp != null) {
            st.append(temp.getData().toString());
            temp = temp.getLink();
        }
        return st.toString();
    }
    ...
    {
        ObjectNode head = new ObjectNode(new String(new byte[]{'A'}), null);
        ObjectNode temp = head;
        for (int i = 1; i < 26; i++) {
            temp.addNodeAfter(new String(new byte[]{(byte)('A'+i)}));
            temp = temp.getLink();
        }
        ...
        System.out.println("Number of nodes = " + listLength(r));
        System.out.println("Number of nodes = " + listLength_rec(r));
        ObjectNode r2 = listCopy_rec(head);
        System.out.println(r2);
        System.out.println("Number of nodes = " + listLength(r2));
        System.out.println("Number of nodes = " + listLength_rec(r2));
